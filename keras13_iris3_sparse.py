...
#3. Compile, Train
model.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['acc']) # 'sparse_categorical_crossentropy'가 node의 값이 다양한 숫자가 나오면 1이나 0으로 만들어준다
# 지금은 y의 원핫인코딩을 하지 않았다.(keras13_iris2_onehot와 다른점) 즉 sparse_categorical_crossentropy 이게 더 편하긴 함(원핫인코딩을 포함함)
