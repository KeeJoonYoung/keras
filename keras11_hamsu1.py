# 함수형 모델 작성에 대해 알아본다 (N : M 일때의 함수형 모델)
...
# 모델 구성
input1 = Input(shape=(5,)) # input 이라는 별도의 layer 를 생성
dense1 = Dense(3)(input1) # hidden layer 의 첫번째 layer
dense2 = Dense(4)(dense1)
dense3 = Dense(5)(dense2) # 이전 레이어를 뒤에다 붙여서 늘리는 방식
output1 = Dense(2)(dense3) # 마지막 Dense 의 값은 output node 가 2개란 소리
...
'''
    Sequential 모델의 문제점
    순차적으로 layer 를 거치기 때문에 
    중간의 layer 를 수정하거나 뭔가를 추가 하는것이 불가능
    함수형 모델은 Sequential 과는 다르게
    input 을 시작하는 지점에서 명시해야함
    input 이란 것을 tensorflow.keras.layers 에서 가져오고 (함수형에서 쓸 input layer 를 지정)
